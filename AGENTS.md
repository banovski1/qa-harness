...
Follow existing two-space indentation, single quotes, semicolons, and ES modules (`.mjs` for tooling). Use camelCase functions and PascalCase classes. Preserve LF line endings. No dedicated formatter or linter is configured.
...
Analyzer tests use `node:test` and `node:assert/strict`, with `__tests__/*.test.mjs`, fixture apps, and report snapshots. Add regression cases for parser changes. Playwright scenarios use `tests/e2e/<module>/*.spec.ts`; keep locators in page objects and prefer web-first assertions over fixed sleeps. No numeric coverage threshold is configured. Validate generator changes with the analysis gate, dry run, and generated-output diff.
