...
//
//   node scripts/framework-generator/check-analysis.mjs [--strict <route>...]
//
...
// Importing generate.mjs to reuse loadConfig would run the generator as a side effect, so
// the two files read the config independently — the same trade check-api-map.mjs makes.
...
const config = {
  analysisDir: rawConfig.analysisDir ?? 'analysis',
...
const appPath = routeData.app ?? componentData.app;
if (!appPath) {
...
for (const route of routeData.routes ?? []) {
  ...
  const strict = strictTargets.has(path);
  const say = (message) => (strict ? failures : warnings).push(`${path}: ${message}`);
...
for (const component of componentData.components ?? []) {
  ...
  for (const element of component.elements ?? []) {
    ...
    if (element.locator.strategy === 'template') templatesUsed.add(element.locator.args?.[0]);
    try {
      fromMap(element.locator, config.locatorTemplates);
    } catch (error) {
      failures.push(`${component.file}: element '${element.name}' has an unusable locator — ${error.message}`);
    }
  }
}
...
for (const entry of config.navigation) {
  ...
  try {
    ...
    failures.push(`navigation: '${entry.name}' has an unusable locator — ${error.message}`);
  }
}
...
try {
  ...
  failures.push(`the generator cannot build a model from this analysis: ${error.message}`);
}
...
function readAnalysis(name) {
  try {
    return JSON.parse(readFileSync(join(config.analysisDir, name), 'utf8'));
  } catch (error) {
