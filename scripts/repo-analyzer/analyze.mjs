...
export function analyzerPlan(args) {
  ...
  return [
    {name: 'detect', args: [path.join(HERE, 'detect.mjs')]},
    {name: 'routes', args: [path.join(HERE, 'routes.mjs')]},
    {name: 'components', args: [path.join(HERE, 'components.mjs')]},
    {name: 'api-docs', args: [path.join(HERE, 'api-docs.mjs')]},
    {name: 'live-urls', args: [path.join(HERE, 'live-urls.mjs'), ...liveUrlArgs]},
  ];
}
