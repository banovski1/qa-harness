...
| `positional-locator` | `nth-child`, `.nth(`, `.first()`, `text=`, framework class selectors | a locator from the ladder — test id, named role, label, or the label template (see `scripts/framework-generator/locator-ladder.mjs`) |
