...
| Assertion/save fails with an "already exists" / duplicate-value server error | Test data collision (e.g. reused username across runs) | The spec's own generated test data | Seed the field with `uniqueUsername`/`uniqueEmail`/`uniqueSuffix` from `src/utils/testData.ts`. Not `Date.now()` — two workers can start in the same millisecond |
